...
public final class CellCommands implements CommandExecutor, TabCompleter {
    ...
    private boolean handleInfo(Player player, String[] args) {
        if (!PermissionManager.checkPermission(player, Constants.Permissions.CELL_INFO)) {
            return true;
        }
        if (args.length < 2) {
    ...
    private boolean handleList(Player player, String[] args) {
        if (!PermissionManager.checkPermission(player, Constants.Permissions.CELL_LIST)) {
            return true;
        }
        var cells = plugin.getCellManager().getPlayerCells(player);
    ...
    private boolean handleAddMember(Player player, String[] args) {
        if (!PermissionManager.checkPermission(player, Constants.Permissions.CELL_ADD_MEMBER)) {
            return true;
        }
        if (args.length < 3) {
    ...
    private boolean handleRemoveMember(Player player, String[] args) {
        if (!PermissionManager.checkPermission(player, Constants.Permissions.CELL_REMOVE_MEMBER)) {
            return true;
        }
        if (args.length < 3) {
